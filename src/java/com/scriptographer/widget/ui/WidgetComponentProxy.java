...
package com.scriptographer.widget.ui;
...
import com.scriptographer.widget.FrameStyle;
import com.scriptographer.widget.Item;
...
import com.scriptographer.widget.TextEditItem;
import com.scriptographer.widget.TextOption;
...
import com.scriptographer.widget.ValueItem;
 ...
 *
 */
public class WidgetComponentProxy extends ComponentProxy {
	...
	public WidgetComponentProxy(Component component) {
		super(component);
	...
	protected Item createItem(Dialog dialog) {
			...
			if (steppers != null && steppers) {
				item = new SpinEdit(dialog) {
					protected void onChange() {
						WidgetComponentProxy.this.onChange(true);
					}
				};
			} else {
		...
		case BUTTON: {
			item = new Button(dialog) {
				protected void onClick() {
					WidgetComponentProxy.this.onClick();
				}
			};
		}
		break;
...
*/		
			...
			if (menu != null) {
				entry = new ListEntry(menu) {
					protected void onSelect() {
						WidgetComponentProxy.this.onSelect();
					}
				};
	...
	protected int addToContent(Dialog dialog,
			LinkedHashMap<String, com.scriptographer.widget.Component> content,
			int column, int row) {
