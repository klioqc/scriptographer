 ...
 * 
 */
public class WidgetPaletteProxy extends PaletteProxy {

	private FloatingDialog dialog;

	public WidgetPaletteProxy(Palette palette, Component[] components) {
		super(palette);
	...
	protected static TableLayout createLayout(Dialog dialog,
		...
		LinkedHashMap<String, com.scriptographer.widget.Component> content =
				new LinkedHashMap<String, com.scriptographer.widget.Component>();

		int column = hasLogo ? 1 : 0, row = 0;
		for (int i = 0; i < components.length; i++) {
			Component item = components[i];
			if (item != null) {
				WidgetComponentProxy proxy = new WidgetComponentProxy(item);
		...
		if (hasLogo) {
			...
			logo.setImage(WidgetUiFactory.getImage("logo.png"));
