 ...
 * @author lehni
 */
public class ImageBase extends NativeObject {
	// an image can wrap its representation as an icon as well...
	protected int iconHandle = 0;
	...
	public ImageBase(int width, int height, ImageType rgb) {
		this.width = width;
		this.height = height;
		this.type = rgb != null ? rgb : ImageType.RGB;
		handle = nativeCreate(width, height, rgb.value);
	}
	

	public static ImageBase getImage(Object obj) throws IOException {
		if (obj instanceof ImageBase)
			return (ImageBase) obj;
		else if (obj instanceof String)
			return new ImageBase((String) obj);
		else if (obj instanceof File)
			return new ImageBase((File) obj);
		else if (obj instanceof URL)
			return new ImageBase((URL) obj);
		else
			return null;
	}
	
	public ImageBase(java.awt.Image image) {
		BufferedImage buf;
	...
	public ImageBase(Raster raster) {
	...
	// too:
	public ImageBase(File file) throws IOException {
//		this(checkImage(ImageIO.read(file), file));
		this(checkImage(waitForImage(Toolkit.getDefaultToolkit().createImage(
				file.getPath())), file));
	}

	public ImageBase(ImageProducer producer) throws IOException {
		this(checkImage(waitForImage(Toolkit.getDefaultToolkit().createImage(
				producer)), producer));
	}

	public ImageBase(URL url) throws IOException {
//		this(checkImage(ImageIO.read(url), url));
		this(checkImage(waitForImage(Toolkit.getDefaultToolkit().createImage(
				url)), url));
	}

	public ImageBase(String str) throws IOException {
		this(getURL(str));
	}
	
	public ImageBase(InputStream in) throws IOException {
...
//	}
	...
	protected  int nativeCreate(int width, int height, int type){return 0; };

	protected  void nativeDestroy(int handle, int iconHandle){};

	protected  void nativeSetPixels(int[] data, int width, int height,
			int byteWidth) {};

			protected  void nativeGetPixels(int[] data, int width, int height,
			int byteWidth) {};

			protected  void nativeSetPixels(int handle, int numBytes){};

			protected  int nativeCreateIcon() {return 0;}
